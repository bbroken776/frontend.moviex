...
const MoviesDisplayPagination = ({ currentPage, totalPages, onPageChange }: MoviesDisplayPaginationProps) => {
  ...
  return (
    <div className="flex justify-center mt-28 gap-4 items-center">
      {startPage > 1 && (
        <button
          onClick={() => onPageChange(1)}
          className="px-4 py-2 rounded font-bold text-sm transition-colors ease-in-out duration-300 bg-transparent text-zinc-100 hover:bg-amber-500/20 hover:text-amber-500"
        >
          1
        </button>
      )}
      ...
      {pageNumbers.map(page => (
        <button
          ...
          className={`px-4 py-2 rounded font-bold text-sm transition-colors ease-in-out duration-300 ${
